...
def parse_args():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "关闭在线随机 payload，改为读取 reference/ 中已有的固定 cfile；"
            "仅用于对照。"
        ),
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    # OTA checkpoint 默认不是同名；当前脚本没有显式的 --pretrained 参数。
    if OTA:
    ...
    if OTA:
        ...
        # 四层 model0 的训练循环固定解包 (x, y, snr)，所以这里必须
        # return_snr=True。普通 model0 虽接收 snr 参数，但 gated=False
        # 时实际上不会使用该数值。
        ...
        # OTALoRaDataset 默认 training=True，只迭代 80% 训练索引；当前
        # nn.py 没有建立 training=False 的验证/测试 DataLoader。
        print(f"LoRa, size={dataset.size}, ota_root={OTA_ROOT}")
    else:
    ...
    # reference_phy 默认在每次 __getitem__ 时重新生成 payload 和 AWGN；
    # fixed reference/upstream 数据集只会改变每个 epoch 的读取顺序。
    dataloader = DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True)

    # 若 checkpoints/ 下已有完全同名权重，则从该权重继续训练。
    # 因为当前只支持“精确同名续训”，论文的 synthetic -> OTA 两阶段迁移
    # 并没有被做成一个独立、明确的命令行接口。
    loss_history, model = load_existing_state(model_name, model)

    # 标准监督训练：低速率带噪 IQ -> RF-SR -> 高速率干净 IQ 标签。
    # loss_history = []
